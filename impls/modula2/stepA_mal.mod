...
BEGIN
  ...
  ELSIF (curr # NIL) AND IsSpecialForm(curr^.val, "defmacro!") THEN
    (* defmacro! special form: (defmacro! name (params...) body) *)
      ...
      (* Save the old metadata before creating a copy *)
      oldMeta := evaledItem^.meta;

      (* Create a copy of the function to avoid mutating the original *)
      evaledItem := NewFunction(evaledItem^.params, evaledItem^.body, evaledItem^.closure, TRUE);

      (* Restore metadata *)
      evaledItem^.meta := oldMeta;
...
BEGIN
  ...
  Assign('(def! *host-language* "modula-2")', loadCmd);
  output := rep(loadCmd);
